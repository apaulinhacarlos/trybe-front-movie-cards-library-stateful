...
Neste projeto eu desenvolvi uma aplicação que consiste em uma biblioteca de cartões de filmes dinâmica utilizando React. A biblioteca é composta por:
...
```bash
git clone git@github.com:apaulinhacarlos/trybe-front-movie-cards-library-stateful/.git
```

Acesse o diretório do projeto com o comando `cd`:

```bash
cd trybe-front-movie-cards-library-stateful
```
